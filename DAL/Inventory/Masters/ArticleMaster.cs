...
namespace ERPSample.DAL.Inventory.Masters
{
    public class ArticleMaster
    {
        ...
        public ArticleMaster(String ConnectionString = "")
        {
        ...
        {
            SqlCommand Cmd = new SqlCommand("MaMiscArticleMasterSP", new SqlConnection(ConnectionString));
        ...
        public int CheckDuplicateEntry(string Value)
        {
            try
            {
                ...
                Cmd.Connection.Open();
                Cmd.CommandText = "SELECT Count(*) As Count FROM MaMisc WHERE [Key]='Item Article' and Value=@Value";
        ...
        public string InsertArticleMaster(MaMisc ma, SqlConnection Conn, SqlTransaction Tx)
        {
            try
            {
                ...
                {
                    Cmd.Parameters.AddWithValue("@Key", "Item Article");
                }
                Cmd.Connection.Open();
        ...
        public int DeleteArticleMaster(int ID)
        {
            try
            {
                SqlCommand Cmd = new SqlCommand("MaMiscArticleMasterSP", new SqlConnection(ConnectionString));
