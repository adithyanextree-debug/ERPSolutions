...
{
    public class ArticleMasterController : BaseController
    {
        private readonly Microsoft.AspNetCore.Hosting.IWebHostEnvironment _appEnvironment;
        //private readonly string baseUrl;

        public ArticleMasterController(Microsoft.AspNetCore.Hosting.IWebHostEnvironment appEnvironment)
        {
        ...
        private DAL.Inventory.Masters.ArticleMaster _ArticleMaster;

        private DAL.Inventory.Masters.ArticleMaster ArticleMaster
        {
            get
            {
                if (_ArticleMaster == null)
                {
                    _ArticleMaster = new DAL.Inventory.Masters.ArticleMaster(ConnectionString);
                }
        ...
        public async Task<IActionResult> Index(int MenuID)
        {
            ...
            DataTable dt = ArticleMaster.Fill();
            ...
            ViewBag.ArticleMaster = brands;
            ViewBag.MenuID = MenuID;
            return View("~/Views/Invertory/Masters/ArticleMaster.cshtml");
        }

        public async Task<IActionResult> NewEntry()
        {
            DataTable dt = ArticleMaster.SelectCode();
        ...
        public async Task<IActionResult> SaveEntry(MaMisc ma)
        {
            ...
            try
            {
                ...
                {
                    int count = ArticleMaster.CheckDuplicateEntry(ma.Value);
                    if (count > 0)
                    {
                        return Json(new { success = false, message = "This Article Already Exists!" });

                    }
                    else
                    {
                        // Value is not a duplicate, return JSON response indicating not a duplicate
                        Result = ArticleMaster.InsertArticleMaster(ma, Con, Tx);
                    }
                }
                else
                {
                ...
                Tx.Commit();
                Con.Close();
                return Json(new { success = true, message = "Article added", transactionNo = ma.ID });
        ...
        public async Task<IActionResult> RowClick(int ID)
        {
            ...
            DataTable Dt = ArticleMaster.RowClick(ID);
        ...
        {
            int val = ArticleMaster.DeleteArticleMaster(ID);
            if (val != 0)
            {
